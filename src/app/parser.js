...
Parse.Parser = declare(null, {
                ...
                for(var i = 0; i < this.children.length; ++i){
                    if(r = this.children[i].parse(tokens)){
                        ...
                        if(tokens.length){
                            ...
                            parser.errors.push({
                                cause : this.type,
                                msg : "Unexpected syntax. Expected : " + this.children[i].type,
    ...
    Token: function(tokentype){
        return {
            ...
            parse: function(tokens){
                ...
                if(tokens.length){
                    var token = tokens[0];
                    this.parser.errors.push({
                        cause : this.type,
                        msg : "Unexpected token. Expected : " +  this.tokentype,
                        line_num : token.line_num,
                        pos : token.col
                    });
                }
    ...
    Maybe: function(child) {
        ...
        return {
            ...
            parse: function(tokens){
                var r = this.child.parse(tokens);
                return r || {rests : tokens, value : null, group : $H({})};
            }
        };
    },
    End: function(){
        ...
        return {
            ...
            parse: function(tokens){
               if(!tokens.length){
                    ...
                    parser.errors.push({
                        cause : this.type,
                        msg : "Expected EOF, but I'm still in middle of the inputs!",
                        line_num : token.line_num,
                        pos : token.col
                    });
                   return null;
